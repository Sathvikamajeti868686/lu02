...
class App extends Component {
  constructor(){
    super();
    this.state={formData:{
      firstName: "",   /* mapping state with input line 35*/
      lastName: "",
      email: "",
      phone :"",
      country:"",
      ...
      confirmpassword:"",
      terms:false,
    }, errMsg:{}}; /*{}=> dictionary(for multiple values) having key and value */
  }
  handleSubmit(){
    if(this.validate())
     // return;
  ...
  handleInput(e){
    this.setState({formData:{...this.state.formData,[e.target.name]:e.target.value}}); /* ... (spread operator)  =>copy the contet of current state */
  }
  validate(){
    const {formData} = this.state;
    let newErrors={};
    if(!formData.firstName.trim()) /* trim => any white space extra added on bothsides (prefix & suffix) it will remove */
    newErrors.firstName="FirstName is required";
     ...
     if(!formData.country.trim()) 
      newErrors.country="Select the country";
    if(!formData.dob.trim()) 
      newErrors.dob="Data of Birth is required";
    if(formData.password.length < 8)
      newErrors.password = "Password must have 8 chars";
    if(formData.password!== formData.confirmpassword)
      newErrors.confirmpassword="password not matched";
    if(!formData.terms)
      newErrors.terms="Accept terma and conditions";

    this.setState({errMsg: newErrors});
    return Object.keys(newErrors).length === 0;
  }
  render() {
    const{formData, errMsg}=this.state; /* after ading this our o/p is blank bcz we didn,t decleare any state so intialize stae */
    return (
      <>
        <header>
